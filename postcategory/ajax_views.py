...
class AjaxDeleteView:
    def get(self, request, *args, **kwargs):
        ...
        self.ajax_list = 'ajax/todo_list_partial.html'
        if request.is_ajax():
            print('ggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg123')
            try:
                html_form = render_to_string(
                    self.ajax_partial, context, request)
            except template.TemplateDoesNotExist:
                html_form = render_to_string(
                    'ajax/delete_partial.html', context, request)
    ...
    def post(self, *args, **kwargs):
        ...
        self.ajax_list = 'ajax/todo_list_partial.html'
        if request.is_ajax():
            print(data,'ssssssssssssskkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk232323232')
            self.object = self.get_object()
            self.object.delete()
            data = dict()
            data['form_is_valid'] = True
            ...
            try:
                data['list'] = render_to_string(
                    self.ajax_list, context, self.request)
                print(data,'ssssssssssssskkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk')
            except template.TemplateDoesNotExist:
                data['list'] = render_to_string(
                    'ajax/list_partial.html', context, self.request)
            return JsonResponse(data)
        else:
            print('ssssssssssssskkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk22222222222')
